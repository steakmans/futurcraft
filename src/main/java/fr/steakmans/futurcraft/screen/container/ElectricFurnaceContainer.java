...
import fr.steakmans.futurcraft.screen.container.syncdata.ElectricFurnaceContainerData;
...
public class ElectricFurnaceContainer extends AbstractContainerMenu {
    ...
    //client
    protected ElectricFurnaceContainer(int id, Inventory playerInv) {
        this(id, playerInv, new ItemStackHandler(7), BlockPos.ZERO, new SimpleContainerData(4));
    }

    //server
    public ElectricFurnaceContainer(int id, Inventory playerInv, IItemHandler slots, BlockPos pos, ContainerData data) {
        super(ModContainers.ELECTRIC_FURNACE.get(), id);
        ...
        addSlot(new SlotItemHandler(slots, 2, startX, 10));
    ...
    public boolean stillValid(Player player) {
        return stillValid(containerAcces, player, ModBlocks.ELECTRIC_FURNACE.get());
    }

    public static MenuConstructor getServerContainer(TileEntityElectricFurnace be, BlockPos pos) {
        return (id, playerInv, player) -> new ElectricFurnaceContainer(id, playerInv, be.inventory, pos, new ElectricFurnaceContainerData(be, 4));
    }
